...
+ pause() : void \\
+ loadLevel(id:int) : void
}

% --- Classe Player ---
\umlclass[x=-5,y=-5]{Player}{
- money : int \\
- lives : int
}{
...
% --- Classe Level ---
\umlclass[x=5,y=-5]{Level}{
- map : Map \\
- waves : List<Wave> \\
- enemies : List<Enemy>
}{
+ startWave(index:int) : void \\
+ update(dt:float) : void \\
+ isCleared() : bool
}

% --- Classe Map ---
\umlclass[x=10,y=-10]{Map}{
- grid : Tile[][] \\
- paths : List<Path>
}{
+ getTile(x:int,y:int) : Tile \\
+ findPath(start:Tile,end:Tile) : Path
}
...
% --- Classe Tower ---
\umlclass[x=-5,y=-10]{Tower}{
- range : float \\
- fireRate : float \\
- level : int
}{
+ upgrade() : void \\
+ canShoot(e:Enemy) : bool \\
+ shoot(e:Enemy) : Projectile
}

% --- Classe Projectile ---
\umlclass[x=-10,y=-15]{Projectile}{
- damage : int \\
- velocity : Vector
}{
+ update(dt:float) : void \\
...
% Relations
\umlcompo{Game}{Player}
\umlcompo{Game}{Level}
\umlcompo{Level}{Map}
\umlcompo{Level}{Enemy}
\umlcompo{Tower}{Projectile}
\umluniassoc{Projectile}{Enemy}

\end{tikzpicture}
\caption{Diagramme UML des classes principales du Tower Defense}
\end{figure}
