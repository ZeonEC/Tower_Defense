...
RenderMap::RenderMap(const sf::Vector2u& windowSize) {
    ...
    if (!backgroundTexture.loadFromFile("../src/assets/maps/map_forest.jpeg")) {
        std::cerr << "Erreur : impossible de charger le background\n";
    }
    background.setTexture(backgroundTexture);
...
void RenderInfo::drawInfo() {
    ...
    if (player) {
        ...
        float healthPercent = static_cast<float>(hp) / 20.f; //variable a changer pour l'affichage initiale de la vie
        ...
        healthText.setString(std::to_string(hp) + " / 20"); //valeur de comparaison dans la barre de vie
